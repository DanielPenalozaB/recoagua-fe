...
export const guideKeys = {
  ...
  progress: (guideId: number, userId: number) =>
    [...guideKeys.detail(guideId), "progress", userId] as const,
...
export const useCreateGuide = () => {
  ...
  return useMutation({
    mutationFn: (guideData: CreateGuideDto) =>
      guideService.createGuide(guideData),
    onSuccess: () => {
...
export const useGuideProgress = (guideId: number, userId: number) => {
  return useQuery<ApiResponse<ModuleProgress[]>>({
    // Explicitly define the return type as an array
