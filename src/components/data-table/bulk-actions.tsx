...
export function DataTableBulkActions<TData>({
	...
	useEffect(() => {
		if (selectedCount > 0) {
			const message = `${selectedCount} ${entityName}${selectedCount > 1 ? "s" : ""} seleccionado. Acciones masivas disponibles.`;
	...
	const handleKeyDown = (event: React.KeyboardEvent) => {
		...
		const currentIndex = Array.from(buttons).indexOf(
			(document.activeElement as HTMLButtonElement) || buttons[0],
		);
	...
	return (
		<>
			...
			<div
				...
				className={cn(
					"bottom-6 left-1/2 z-50 fixed rounded-xl -translate-x-1/2",
				...
				>
					...
					>
						<Badge
							...
							aria-label={`${selectedCount} ${entityName} ${selectedCount > 1 ? "seleccionados" : "seleccionado"}`}
						>
							{selectedCount}
						</Badge>{" "}
						...
						{selectedCount > 1 ? "seleccionados" : "seleccionado"}
					</div>
