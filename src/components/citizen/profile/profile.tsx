...
const changePasswordFormSchema = z.object({
  currentPassword: z.string().min(6, 'La contraseña debe tener al menos 6 caracteres'),
  newPassword: z.string().min(6, 'La contraseña debe tener al menos 6 caracteres'),
      ...
      {isChangingPassword ? (
        <Form {...form}>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='Contraseña actual' type='password' {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='Nueva contraseña' type='password' {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder='Confirmar contraseña' type='password' {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <Button type='submit' disabled={!form.formState.isValid}>Actualizar</Button>
            </div>
          </form>
        </Form>
      ) : (
        ...
        <div className="flex max-sm:flex-col justify-between sm:items-center gap-4 bg-white shadow-sm p-6 rounded-lg">
          <div className="flex items-center gap-4">
            <span className="flex justify-center items-center bg-teal-600 rounded-full size-16 font-bold text-white text-2xl">
              {user.image ? (
                <img src={user.image} alt={user.name || 'User'} className="rounded-full w-16 h-16 object-cover" />
              ) : (
                getInitials(user.name || '')
              )}
            </span>
            <div className="flex flex-col">
              <span className="font-bold text-[#115E59] text-lg">{user.name}</span>
              <span className="font-medium text-neutral-500 text-base">{user.email}</span>
              {user.city && (
          ...
          {!isChangingPassword && <Button variant='outline' onClick={() => setIsChangingPassword(true)}>Cambiar contraseña</Button>}
        </div>
