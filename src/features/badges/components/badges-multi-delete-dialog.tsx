...
type BadgesMultiDeleteDialogProps<TData> = {
...
export function BadgesMultiDeleteDialog<TData>({
	open,
	onOpenChange,
	table,
}: BadgesMultiDeleteDialogProps<TData>) {
	...
	const handleDelete = () => {
		...
		toast.promise(sleep(2000), {
			loading: "Eliminando insignias...",
			success: () => {
				table.resetRowSelection();
				return `Eliminar ${selectedRows.length} ${
					selectedRows.length > 1 ? "insignias" : "insignia"
				}`;
			},
			error: "Error",
		});
		...
		<ConfirmDialog
			...
			desc={
				<div className="space-y-4">
					<p className="mb-2">
						Estas seguro de eliminar {selectedRows.length} insignias? <br />
