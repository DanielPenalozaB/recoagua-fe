...
type RegionMultiDeleteDialogProps<TData> = {
...
export function RegionsMultiDeleteDialog<TData>({
	open,
	onOpenChange,
	table,
}: RegionMultiDeleteDialogProps<TData>) {
	...
	const handleDelete = () => {
		...
		toast.promise(sleep(2000), {
			loading: "Eliminando regiones...",
			success: () => {
				table.resetRowSelection();
				return `Eliminar ${selectedRows.length} ${
					selectedRows.length > 1 ? "regiones" : "región"
				}`;
			},
			error: "Error",
		});
		...
		<ConfirmDialog
			...
			desc={
				<div className="space-y-4">
					<p className="mb-2">
						Estas seguro de eliminar {selectedRows.length} regiones? <br />
