...
const useLevel = (id: number | undefined) => {
	return useQuery({
		queryKey: id ? levelKeys.detail(id) : levelKeys.details(),
		queryFn: () => levelService.getLevel(id as number),
		enabled: !!id,
	});
};
...
const useCreateLevel = () => {
	...
	return useMutation({
		mutationFn: (userData: CreateLevelDto) =>
			levelService.createLevel(userData),
		onSuccess: () => {
...
export { useCreateLevel, useDeleteLevel, useUpdateLevel, useLevel, useLevels };
