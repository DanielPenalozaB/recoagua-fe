...
import {
	...
	useCreateCity,
	useUpdateCity,
} from "@/features/cities/hooks/use-city";
...
export function CitiesCreateEditForm({ cityId }: CitiesCreateEditFormProps) {
	...
	const onSubmit = async (formData: CityFormValues) => {
		...
		try {
			...
			const rainfall =
				formData.rainfall && formData.rainfall !== ""
					? Number.parseFloat(formData.rainfall.replace(",", "."))
					: undefined;
			...
			const cityData = {
				...
				rainfall,
				regionId: Number.parseInt(formData.regionId, 10),
				language: "es",
			};
	...
	}, [getCityMutation?.isSuccess, form.setValue, getCityMutation?.data?.data]);
								...
								<Select onValueChange={field.onChange} value={field.value}>
									<FormControl>
										<SelectTrigger className="w-full bg-white">
											<SelectValue
												placeholder={
													isLoading ? "Cargando regiones..." : placeholderText
												}
											/>
										</SelectTrigger>
									</FormControl>
