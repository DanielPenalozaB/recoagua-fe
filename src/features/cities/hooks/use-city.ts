...
import type { CityFilterDto, CreateCityDto, UpdateCityDto } from "@/types/city";
...
export const useCity = (id: number | undefined) => {
	return useQuery({
		queryKey: id ? cityKeys.detail(id) : cityKeys.details(),
		queryFn: () => cityService.getCity(id as number),
		enabled: !!id,
	});
};
