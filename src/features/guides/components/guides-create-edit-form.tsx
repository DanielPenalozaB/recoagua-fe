...
import { Plus, Trash2, GripVertical, ChevronDown, ChevronRight, AlertCircle, CheckCircle2, ExternalLink } from 'lucide-react'
...
const guideFormSchema = z.object({
  name: z.string().min(2, 'El nombre debe tener al menos 2 caracteres.').max(45, 'El nombre debe tener menos de 45 caracteres.'),
...
export function GuidesCreateEditForm({ guideId }: GuideCreateEditFormProps) {
  ...
  const [mediaPreview, setMediaPreview] = useState<{ [key: string]: { isValid: boolean; type: string } }>({})
  ...
  const validateMediaUrl = (blockId: string, url: string, type: 'video' | 'image') => {
    if (!url) {
      setMediaPreview(prev => ({ ...prev, [blockId]: { isValid: false, type } }))
      return
    }
  ...
  const validateBlock = (block: Block): { isValid: boolean; errors: string[] } => {
    const errors: string[] = []
    ...
    switch (block.type) {
        ...
        if (block.questionType === 'matching') {
          ...
          if (block.questionType === 'true_false') {
            if (block.answers.length !== 2) {
              errors.push('Verdadero/Falso debe tener exactamente 2 respuestas')
            }
      ...
      case 'interactive':
        ...
        if (block.dynamicType === 'matching' && block.relationalPairs.length < 2) {
          errors.push('Se requieren al menos 2 pares para emparejamiento')
        }
        break
    }
    ...
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? { 
              ...module, 
              blocks: [...module.blocks, { ...newBlock, order: module.blocks.length + 1 }] 
            }
          : module
      )
    )
  ...
  const updateBlock = (moduleId: string, blockId: string, field: string, value: any) => {
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? {
              ...
              blocks: module.blocks.map(block => {
                if (block.id === blockId) {
                  ...
                  if (field === 'questionType' && block.questionType === 'matching') {
                    updatedBlock.relationalPairs = []
                  }
  ...
  const updateAnswer = (moduleId: string, blockId: string, answerId: string, field: string, value: any) => {
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? {
  ...
  const deleteAnswer = (moduleId: string, blockId: string, answerId: string) => {
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? {
              ...
              blocks: module.blocks.map(block =>
                block.id === blockId
                  ? { ...block, answers: block.answers.filter(answer => answer.id !== answerId) }
                  : block
              ),
            }
          : module
  ...
  const updateRelationalPair = (moduleId: string, blockId: string, pairId: string, field: string, value: any) => {
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? {
  ...
  const deleteRelationalPair = (moduleId: string, blockId: string, pairId: string) => {
    setModules(prev =>
      prev.map(module =>
        module.id === moduleId
          ? {
              ...
              blocks: module.blocks.map(block =>
                block.id === blockId
                  ? { ...block, relationalPairs: block.relationalPairs.filter(pair => pair.id !== pairId) }
                  : block
              ),
            }
          : module
  ...
  const calculateTotalPoints = () => {
    return modules.reduce((total, module) => {
      return total + module.blocks.reduce((blockTotal, block) => blockTotal + block.points, 0)
    }, 0)
  }
...
const onSubmit = async (formData: GuideFormValues) => {
  if (!canSubmitForm()) {
    alert('Por favor, completa todos los campos requeridos y corrige los errores de validación.')
    return
  }
  ...
  try {
    ...
    const guideData: CreateGuideDto = {
      ...
      modules: modules.map((module, moduleIndex): CreateModuleDto => ({
        name: module.name,
        description: module.description,
        order: module.order,
        points: module.points,
        status: module.status as ModuleStatus,
        blocks: module.blocks.map((block, blockIndex): CreateBlockDto => ({
          ...
          answers: block.answers?.map((answer, answerIndex): CreateAnswerDto => ({
            text: answer.text,
            isCorrect: answer.isCorrect,
            feedback: answer.feedback || undefined,
            order: answer.order
          })) || undefined,
          relationalPairs: block.relationalPairs?.map((pair, pairIndex): CreateRelationalPairDto => ({
    ...
    alert('Error al guardar la guía. Por favor, verifica que todos los campos estén completos.')
  } finally {
      ...
      if (guide.modules) {
        ...
        const localModules = guide.modules.map((module: any) => ({
          ...
          blocks: module.blocks?.map((block: any) => ({
            ...block,
            id: `block-${block.id}`,
            answers: block.answers?.map((answer: any) => ({
              ...answer,
              id: `answer-${answer.id}`
            })) || [],
            relationalPairs: block.relationalPairs?.map((pair: any) => ({
              ...pair,
              id: `pair-${pair.id}`
            })) || []
          })) || []
            ...
            <Tabs defaultValue="content" className="w-full">
              ...
              <TabsContent value="settings" className="space-y-4">
                          ...
                          <SelectContent>
                            {Object.keys(GuideDifficulty).map((difficulty) => (
                              <SelectItem key={difficulty} value={difficulty.toLowerCase()}>
                                {renderDifficultyLabel(difficulty as GuideDifficulty)}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                ...
                <div className="flex items-center gap-4 p-4 bg-muted rounded-lg">
                  ...
                  <div className="text-xs text-muted-foreground">(Calculado automáticamente)</div>
                </div>
              </TabsContent>
            </Tabs>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            {modules.length === 0 ? (
              <Alert>
                ...
                <AlertDescription>
                  No hay módulos creados. Agrega al menos un módulo con bloques para crear la guía.
                </AlertDescription>
              </Alert>
            ) : (
              modules.map((module, moduleIndex) => {
                const moduleHasErrors = !module.name.trim() || module.blocks.length === 0
                
                return (
                  <Card key={module.id} className={`border-l-4 ${moduleHasErrors ? 'border-l-red-500' : 'border-l-blue-500'}`}>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <CardTitle className="text-lg">
                                Módulo {moduleIndex + 1}: {module.name || "Sin título"}
                              </CardTitle>
                              {moduleHasErrors && (
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <Badge variant="outline">{module.blocks.length} bloques</Badge>
                              <Badge variant="secondary">{module.points} puntos</Badge>
                              {moduleHasErrors && (
                    ...
                    {expandedModules.has(module.id) && (
                      <CardContent className="pt-0">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="space-y-2">
                                ...
                                <Input
                                  ...
                                  className={!module.name.trim() ? 'border-red-500' : ''}
                                />
                                {!module.name.trim() && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              <div className="space-y-2">
                                ...
                                <Input
                                  type="number"
                                  value={module.points}
                                  onChange={(e) => updateModule(module.id, "points", parseInt(e.target.value) || 0)}
                                />
                                <p className="text-xs text-muted-foreground">
                                  Puntos adicionales por completar el módulo completo
                                </p>
                              </div>
                              <div className="space-y-2">
                                <Label>Estado</Label>
                        ...
                        <div className="space-y-4">
                          <div className="flex items-center justify-between">
                            <div>
                              <h4 className="text-lg font-semibold">Bloques ({module.blocks.length})</h4>
                              <p className="text-sm text-muted-foreground">
                                Los bloques son los pasos individuales de la guía
                              </p>
                            </div>
                            <div className="flex gap-2">
                              <Select
                                value=""
                                onValueChange={(value) => addBlock(module.id, value as BlockType)}
                              >
                                <SelectTrigger className="w-[180px]">
                                  <SelectValue placeholder="Tipo de bloque" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value={BlockType.TEXT}>📝 Texto</SelectItem>
                                  <SelectItem value={BlockType.VIDEO}>🎥 Video</SelectItem>
                                  <SelectItem value={BlockType.IMAGE}>🖼️ Imagen</SelectItem>
                                  <SelectItem value={BlockType.QUESTION}>❓ Pregunta</SelectItem>
                                  <SelectItem value={BlockType.INTERACTIVE}>🎮 Interactivo</SelectItem>
                                  <SelectItem value={BlockType.QUIZ}>📋 Quiz</SelectItem>
                                </SelectContent>
                              </Select>
                              <Button
                          ...
                          {module.blocks.length === 0 ? (
                            <Alert>
                              <AlertCircle className="h-4 w-4" />
                              <AlertDescription>
                                No hay bloques en este módulo. Agrega al menos un bloque usando el menú desplegable.
                              </AlertDescription>
                            </Alert>
                          ) : (
                            <div className="space-y-3">
                              {module.blocks.map((block, blockIndex) => {
                                const validation = getBlockValidationStatus(block)
                                const preview = mediaPreview[block.id]

                                return (
                                  <Card key={block.id} className={`border-l-4 ${validation.isValid ? 'border-l-green-500' : 'border-l-red-500'}`}>
                                    <CardHeader className="pb-3">
                                      <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3 flex-1">
                                          <Button
                                          ...
                                          <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                              <CardTitle className="text-base">
                                                {renderBlockTypeIcon(block.type)} Bloque {blockIndex + 1}: {block.statement || "Sin título"}
                                              </CardTitle>
                                              {!validation.isValid && (
                                                <AlertCircle className="w-4 h-4 text-red-500" />
                                              )}
                                            ...
                                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                                              ...
                                              {block.type === "question" && block.questionType && (
                                                <Badge variant="secondary">{block.questionType}</Badge>
                                              )}
                                              {block.type === "interactive" && block.dynamicType && (
                                                <Badge variant="secondary">{block.dynamicType}</Badge>
                                              )}
                                              {block.questionType === "matching" && (
                                                <Badge variant="default">{block.relationalPairs.length} pares</Badge>
                                              )}
                                              {block.questionType !== "matching" && block.answers.length > 0 && (
                                                <Badge variant="default">{block.answers.length} respuestas</Badge>
                                              )}
                                              <Badge variant="secondary">{block.points} pts</Badge>
                                              {validation.isValid ? (
                                        ...
                                        <Button
                                          variant="ghost"
                                          size="sm"
                                          onClick={() => deleteBlock(module.id, block.id)}
                                          className="text-destructive hover:text-destructive"
                                          type="button"
                                        >
                                          <Trash2 className="w-4 h-4" />
                                      ...
                                      <CardContent className="pt-0 space-y-4">
                                        ...
                                        <Tabs defaultValue="content" className="w-full">
                                          ...
                                          <TabsContent value="content" className="space-y-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                              <div className="space-y-2">
                                                ...
                                                <Select
                                                  value={block.type}
                                                  onValueChange={(value) => updateBlock(module.id, block.id, "type", value)}
                                                >
                                                  <SelectTrigger>
                                                    <SelectValue />
                                                  </SelectTrigger>
                                              ...
                                              <div className="space-y-2">
                                                ...
                                                <Input
                                                  ...
                                                  value={block.order}
                                                  onChange={(e) =>
                                                    updateBlock(module.id, block.id, "order", parseInt(e.target.value) || 0)
                                                  }
                                                />
                                              </div>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                              ...
                                              <Input
                                                ...
                                                onChange={(e) => updateBlock(module.id, block.id, "statement", e.target.value)}
                                                placeholder="Título o pregunta principal del bloque"
                                                className={!block.statement.trim() ? 'border-red-500' : ''}
                                              />
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                              ...
                                              <Textarea
                                                ...
                                                onChange={(e) => updateBlock(module.id, block.id, "description", e.target.value)}
                                              ...
                                              <div className="space-y-2">
                                                ...
                                                <div className="flex gap-2">
                                                  <Input
                                                    ...
                                                    onChange={(e) => updateBlock(module.id, block.id, "resourceUrl", e.target.value)}
                                                    placeholder={
                                                      block.type === 'video' 
                                                    ...
                                                    className={
                                                      block.resourceUrl && preview && !preview.isValid 
                                                        ? 'border-red-500' 
                                                        : ''
                                                    }
                                                  />
                                                  {block.resourceUrl && (
                                                    <Button
                                                      ...
                                                      variant="outline"
                                                      size="sm"
                                                      onClick={() => window.open(block.resourceUrl, '_blank')}
                                                    >
                                                  ...
                                                  <div className="flex items-center gap-2">
                                                    {preview.isValid ? (
                                                      <>
                                                        ...
                                                        <span className="text-sm text-green-600">URL válida detectada</span>
                                                      </>
                                                    ) : (
                                                      <>
                                                        <AlertCircle className="w-4 h-4 text-red-500" />
                                                        <span className="text-sm text-red-600">
                                                          La URL no parece ser un {block.type === 'video' ? 'video' : 'imagen'} válido
                                                        </span>
                                                      </>
                                                    )}
                                                  </div>
                                                )}
                                                {block.type === 'image' && block.resourceUrl && preview?.isValid && (
                                                  <div className="mt-2 border rounded-lg overflow-hidden">
                                                    <img 
                                                      ...
                                                      onError={() => {
                                                        setMediaPreview(prev => ({
                                                          ...prev,
                                                          [block.id]: { isValid: false, type: 'image' }
                                                        }))
                                                      }}
                                                    />
                                                  </div>
                                            ...
                                            {block.type === 'text' && (
                                              <Alert>
                                                <AlertDescription>
                                                  💡 Este bloque mostrará texto informativo al usuario. Usa la descripción para el contenido principal.
                                                </AlertDescription>
                                              </Alert>
                                            )}
                                            ...
                                            <div className="space-y-2">
                                              ...
                                              <Textarea
                                                ...
                                                onChange={(e) => updateBlock(module.id, block.id, "feedback", e.target.value)}
                                          ...
                                          <TabsContent value="settings" className="space-y-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                              <div className="space-y-2">
                                                ...
                                                <Input
                                                  ...
                                                  value={block.points}
                                                  onChange={(e) =>
                                                    updateBlock(module.id, block.id, "points", parseInt(e.target.value) || 0)
                                                  }
                                                  min="0"
                                                />
                                                <p className="text-xs text-muted-foreground">
                                                  Puntos que el usuario gana al completar este bloque
                                                </p>
                                              </div>
                                              ...
                                              {block.type === "interactive" && (
                                                <div className="space-y-2">
                                                  ...
                                                  <Select
                                                    ...
                                                    onValueChange={(value) => updateBlock(module.id, block.id, "dynamicType", value)}
                                                  >
                                                    <SelectTrigger className={!block.dynamicType ? 'border-red-500' : ''}>
                                                      <SelectValue placeholder="Seleccionar tipo" />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                      <SelectItem value="drag_drop">Arrastrar y Soltar</SelectItem>
                                                      <SelectItem value="matching">Emparejamiento</SelectItem>
                                                      <SelectItem value="sorting">Ordenamiento</SelectItem>
                                                      <SelectItem value="fill_blanks">Llenar Espacios</SelectItem>
                                              ...
                                              {(block.type === "question" || block.type === "quiz") && (
                                                <div className="space-y-2">
                                                  <Label>Tipo de Pregunta *</Label>
                                                  <Select
                                                    value={block.questionType}
                                                    onValueChange={(value) => updateBlock(module.id, block.id, "questionType", value)}
                                                  >
                                                    <SelectTrigger className={!block.questionType ? 'border-red-500' : ''}>
                                                      <SelectValue placeholder="Seleccionar tipo" />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                            ...
                                            {(block.type === "question" || block.type === "quiz") && block.questionType && (
                                              <Alert>
                                                <AlertDescription>
                                                  {block.questionType === 'multiple_choice' && '✓ Permite una o más respuestas correctas'}
                                                  {block.questionType === 'true_false' && '✓ Debe tener exactamente 2 opciones (Verdadero/Falso)'}
                                                  {block.questionType === 'open_ended' && '✓ El usuario escribirá una respuesta libre'}
                                                  {block.questionType === 'matching' && '✓ El usuario emparejará elementos relacionados'}
                                                  {block.questionType === 'ordering' && '✓ El usuario ordenará las respuestas correctamente'}
                                                </AlertDescription>
                                              </Alert>
                                            )}
                                          </TabsContent>
                                        ...
                                        {(block.type === "question" || block.type === "quiz") && block.questionType && block.questionType !== 'open_ended' && (
                                          <>
                                            <Separator className="my-6" />
                                            ...
                                            {block.questionType === "matching" ? (
                                              <div className="space-y-4">
                                                <div className="flex items-center justify-between">
                                                  <div>
                                                    <h5 className="text-base font-semibold">
                                                      Pares Relacionales ({block.relationalPairs.length})
                                                    </h5>
                                                    <p className="text-sm text-muted-foreground">
                                                      Los elementos se mezclarán para que el usuario los empareje
                                                    </p>
                                                  </div>
                                                  <Button
                                                    onClick={() => addRelationalPair(module.id, block.id)}
                                                    size="sm"
                                                    variant="outline"
                                                    type="button"
                                                  >
                                                ...
                                                {block.relationalPairs.length === 0 ? (
                                                  <Alert>
                                                    <AlertCircle className="h-4 w-4" />
                                                    <AlertDescription>
                                                      Agrega al menos 2 pares relacionales para este tipo de pregunta.
                                                    </AlertDescription>
                                                  </Alert>
                                                ) : (
                                                  <div className="space-y-3">
                                                    {block.relationalPairs.map((pair, pairIndex) => (
                                                      <Card key={pair.id} className="p-4">
                                                        <div className="flex items-center gap-4">
                                                          <div className="text-sm font-medium min-w-16">Par {pairIndex + 1}:</div>
                                                          <div className="flex-1 grid grid-cols-2 gap-4">
                                                            <div className="space-y-1">
                                                              ...
                                                              <Input
                                                                ...
                                                                className={!pair.leftItem.trim() ? 'border-red-500' : ''}
                                                              />
                                                            </div>
                                                            <div className="space-y-1">
                                                              <Label className="text-xs text-muted-foreground">
                                                              ...
                                                              <Input
                                                                ...
                                                                className={!pair.rightItem.trim() ? 'border-red-500' : ''}
                                                              />
                                                            </div>
                                                          </div>
                                                          <div className="flex items-center gap-2">
                                                            <Checkbox
                                                              checked={pair.correctPair}
                                                            ...
                                                            />
                                                            <Label className="text-sm whitespace-nowrap">Par Correcto</Label>
                                                          </div>
                                                          <Button
                                                            variant="ghost"
                                                            size="sm"
                                                            onClick={() => deleteRelationalPair(module.id, block.id, pair.id)}
                                                            className="text-destructive hover:text-destructive"
                                                            type="button"
                                                          >
                                                            <Trash2 className="w-4 h-4" />
                                              ...
                                              <div className="space-y-4">
                                                <div className="flex items-center justify-between">
                                                  <div>
                                                    <h5 className="text-base font-semibold">
                                                      Respuestas ({block.answers.length})
                                                    </h5>
                                                    <p className="text-sm text-muted-foreground">
                                                      {block.questionType === 'true_false' && 'Debe tener exactamente 2 respuestas'}
                                                      {block.questionType === 'multiple_choice' && 'Marca una o más respuestas como correctas'}
                                                      {block.questionType === 'ordering' && 'El orden determina la respuesta correcta'}
                                                    </p>
                                                  </div>
                                                  <Button
                                                    onClick={() => addAnswer(module.id, block.id)}
                                                    size="sm"
                                                    variant="outline"
                                                    type="button"
                                                    disabled={block.questionType === 'true_false' && block.answers.length >= 2}
                                                  >
                                                    <Plus className="w-4 h-4 mr-2" />
                                                    Agregar Respuesta
                                                  </Button>
                                                ...
                                                {block.answers.length === 0 ? (
                                                  <Alert>
                                                    <AlertCircle className="h-4 w-4" />
                                                    <AlertDescription>
                                                      Agrega al menos una respuesta para este tipo de pregunta.
                                                    </AlertDescription>
                                                  </Alert>
                                                ) : (
                                                  <div className="space-y-3">
                                                          ...
                                                          <div className="flex items-center gap-4">
                                                            ...
                                                            <div className="flex-1">
                                                              <Input
                                                                ...
                                                                className={!answer.text.trim() ? 'border-red-500' : ''}
                                                              />
                                                            </div>
                                                            {block.questionType !== 'ordering' && (
                                                              <div className="flex items-center gap-2">
                                                                <Checkbox
                                                                ...
                                                                />
                                                                <Label className="text-sm whitespace-nowrap">Correcta</Label>
                                                              </div>
                                                            )}
                                                            <Button
                                                              variant="ghost"
                                                              size="sm"
                                                              onClick={() => deleteAnswer(module.id, block.id, answer.id)}
                                                              className="text-destructive hover:text-destructive"
                                                              type="button"
                                                            >
                                                              <Trash2 className="w-4 h-4" />
                                        ...
                                        {block.type === "interactive" && block.dynamicType === "matching" && (
                                          <>
                                            <Separator className="my-6" />
                                            ...
                                            <div className="space-y-4">
                                              <div className="flex items-center justify-between">
                                                <div>
                                                  ...
                                                  <p className="text-sm text-muted-foreground">
                                                    Para el tipo interactivo de emparejamiento
                                                  </p>
                                                </div>
                                                <Button
                                                  onClick={() => addRelationalPair(module.id, block.id)}
                                              ...
                                              {block.relationalPairs.length === 0 ? (
                                                <Alert>
                                                  <AlertCircle className="h-4 w-4" />
                                                  <AlertDescription>
                                                    Agrega al menos 2 pares para la actividad interactiva.
                                                  </AlertDescription>
                                                </Alert>
                                              ) : (
                                                <div className="space-y-3">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center gap-2">
                  {modules.length > 0 && modules.every(m => m.name.trim() && m.blocks.length > 0) ? (
                    <CheckCircle2 className="w-5 h-5 text-green-500" />
                  ) : (
                    <AlertCircle className="w-5 h-5 text-red-500" />
                  )}
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {modules.filter(m => m.name.trim() && m.blocks.length > 0).length} completos
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  {modules.every(m => m.blocks.every(b => validateBlock(b).isValid)) ? (
                    <CheckCircle2 className="w-5 h-5 text-green-500" />
                  ) : (
                    <AlertCircle className="w-5 h-5 text-red-500" />
                  )}
                  <div>
                    <p className="font-medium">
                      {modules.reduce((acc, m) => acc + m.blocks.length, 0)} Bloques
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {modules.reduce((acc, m) => 
                        acc + m.blocks.filter(b => validateBlock(b).isValid).length, 0
                      )} válidos
                    </p>
                  </div>
                </div>
              ...
              {!canSubmitForm() && (
                <Alert variant="destructive">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Por favor, completa todos los campos requeridos y corrige los errores antes de guardar la guía.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
