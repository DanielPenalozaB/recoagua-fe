...
export function GuidesCreateEditForm({ guideId }: GuideCreateEditFormProps) {
  ...
  const [mediaPreview, setMediaPreview] = useState<{
    [key: number]: { isValid: boolean; type: string };
  }>({});
  ...
  const deleteBlock = (moduleId: string, blockId: number) => {
    setModules((prev) =>
      prev.map((module) =>
        module.id === moduleId
          ? {
  ...
  // Answer management
  const addAnswer = (moduleId: string, blockId: number) => {
    setModules((prev) =>
      prev.map((module) =>
        module.id === moduleId
          ? {
  ...
  const addRelationalPair = (moduleId: string, blockId: number) => {
    setModules((prev) =>
      prev.map((module) =>
        module.id === moduleId
          ? {
                ...
                return (
                  <Card
                    ...
                    className={`border-l-4 ${
                      moduleHasErrors ? "border-l-red-500" : "border-l-blue-500"
                    }`}
                  >
