...
type ZonesMultiDeleteDialogProps<TData> = {
...
export function ZonesMultiDeleteDialog<TData>({
	open,
	onOpenChange,
	table,
}: ZonesMultiDeleteDialogProps<TData>) {
	...
	const handleDelete = () => {
		...
		toast.promise(sleep(2000), {
			loading: "Eliminando zonas...",
			success: () => {
				table.resetRowSelection();
				return `Eliminar ${selectedRows.length} ${
					selectedRows.length > 1 ? "zonas" : "zona"
				}`;
			},
			error: "Error",
		});
		...
		<ConfirmDialog
			...
			desc={
				<div className="space-y-4">
					<p className="mb-2">
						Estas seguro de eliminar {selectedRows.length} zonas? <br />
