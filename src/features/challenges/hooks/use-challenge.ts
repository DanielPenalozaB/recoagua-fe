...
const challengeKeys = {
	...
	list: (filters: ChallengeFilterDto) =>
...
const useChallenge = (id: number | undefined) => {
	return useQuery({
		queryKey: id ? challengeKeys.detail(id) : challengeKeys.details(),
		queryFn: () => challengeService.getChallenge(id as number),
		enabled: !!id,
	});
};
...
const useCreateChallenge = () => {
	...
	return useMutation({
		mutationFn: (userData: CreateChallengeDto) =>
			challengeService.createChallenge(userData),
		onSuccess: () => {
