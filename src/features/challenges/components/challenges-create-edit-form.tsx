...
export function ChallengesCreateEditForm({
	challengeId,
}: ChallengeCreateEditFormProps) {
	...
	useEffect(() => {
		...
		form.setValue,
		getChallengeMutation?.data?.data,
	]);
									...
									<SelectContent>
										{Object.keys(ChallengeType).map((type) => (
											<SelectItem key={type} value={type.toLowerCase()}>
												{renderTypeLabel(type as ChallengeType)}
											</SelectItem>
										))}
									</SelectContent>
								</Select>
									...
									<SelectContent>
										{Object.keys(ChallengeStatus).map((status) => (
											<SelectItem key={status} value={status.toLowerCase()}>
												{renderStatusLabel(status as ChallengeStatus)}
											</SelectItem>
										))}
									</SelectContent>
								</Select>
