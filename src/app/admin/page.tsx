...
import { BarChart3, BookOpen, Calendar, Droplets, Eye, Globe, PieChart, Target, TrendingUp, Trophy, Users } from "lucide-react"
            ...
            <div className='gap-4 grid md:grid-cols-2 lg:grid-cols-4'>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <p className='text-muted-foreground text-xs'>
                    <span className='text-green-600'>+3%</span> desde la semana pasada
                  </p>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {popularGuides.map((guide, index) => (
                      <div key={`participation-guide-${index.toString()}`} className="flex items-center">
                        <div className="flex justify-center items-center bg-teal-100 rounded-lg w-9 h-9 text-teal-600">
                          <BookOpen className="w-5 h-5" />
                        </div>
                        <div className="space-y-1 ml-4">
                          <p className="font-medium text-sm leading-none">{guide.title}</p>
                          <p className="text-muted-foreground text-sm">{guide.completion}% completado</p>
                        </div>
                        <div className="ml-auto font-medium">{guide.users} usuarios</div>
                      </div>
                    ))}
                  </div>
                </CardContent>
                ...
                <CardContent>
                  <div className="space-y-2">
                    ...
                    <div className="bg-gray-200 rounded-full h-2">
                      <div className="bg-teal-600 rounded-full h-2" style={{ width: '65%' }}></div>
                    </div>
                    ...
                    <div className="bg-gray-200 rounded-full h-2">
                      <div className="bg-blue-600 rounded-full h-2" style={{ width: '25%' }}></div>
                    </div>
                    <div className="flex justify-between items-center mt-4">
                    ...
                    <div className="bg-gray-200 rounded-full h-2">
                      <div className="bg-gray-400 rounded-full h-2" style={{ width: '10%' }}></div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-4">
                    {recentContent.map((item, index) => (
                      <div key={index} className="flex items-center">
                        <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${item.type === 'guide' ? 'bg-teal-100 text-teal-600' : item.type === 'module' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'}`}>
                          {item.type === 'guide' ? (
                            <BookOpen className="w-5 h-5" />
                          ) : item.type === 'module' ? (
